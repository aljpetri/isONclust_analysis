...
def plot_memory(data, n_threads):
    ...
    filtered_data = data.loc[data['threads'] == n_threads].copy()
    outfilename = f"MemUsage_t{n_threads}.pdf"

    # Ensure numeric memory values
    ...
    gs = GridSpec(10, 1, figure=fig)  # 10 rows: 9 for plot, 1 for legend

    # Main plot area (takes top 9/10 of vertical space)
    ...
    for p in ax.patches:
        ...
        if not pd.isna(height) and height > 0:
            ax.annotate(
                f'{height:.1f}',
                (p.get_x() + p.get_width() / 2.5, height),
                ha='center',
                va='center',
                fontsize=17,
                color='black',
    ...
    # Axis labels
    ax.set_xlabel('Dataset', fontsize=26)
    ax.set_ylabel('Memory Usage (GB)', fontsize=26)

    # Style ticks
    plt.setp(ax.get_xticklabels(), fontfamily='serif', fontsize=26, weight='normal', color='black', rotation=30, ha='right')
    plt.setp(ax.get_yticklabels(), fontfamily='serif', fontsize=26, weight='normal', color='black')

    # Legend in a dedicated subplot (bottom row)
    handles, labels = ax.get_legend_handles_labels()
    legend_ax = fig.add_subplot(gs[9])
    legend_ax.axis('off')  # Hide actual axes
    legend_ax.legend(
        handles=handles,
        labels=labels,
        title="Tool",
